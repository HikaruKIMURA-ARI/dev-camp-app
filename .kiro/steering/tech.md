...
- **CSS**: pico.css v2（`node_modules` から静的配信、ビルドステップなし）+ `public/app.css` で必要最小限の上書き
...
- `hono/bun` の `serveStatic` — `node_modules` 配下の静的アセット（htmx / Alpine / pico）を直接配信する手段（追加のバンドラを入れない）
...
- **ビルドステップを持たない**: TS / JSX は Bun が直接実行し、CSS と htmx / Alpine の JS は `node_modules` から `serveStatic` で配信する。バンドラを導入しない
- **マイグレーションは起動時に走る**: 運用上の単純さを優先。開発で `db:gen` した SQL は次回起動で自動適用される。生成された SQL は手で書き換えない
